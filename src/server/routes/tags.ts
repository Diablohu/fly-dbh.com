...
import type { VideoItem, ParamsGetVideos } from '@types';
...
function addRoutesTags(router: Router): void {
    router.get('/api/tags', async (ctx) => {
        const { tag, length = 20, start = 0 } = ctx.query as ParamsGetVideos;
